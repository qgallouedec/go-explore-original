...
from goexplore_py.experiment_settings import del_out_of_setup_args, hrv_and_tf_init, parse_arguments, process_defaults, setup
...
CHECKPOINT_ABBREVIATIONS = {"model": MODEL_POSTFIX, "archive": ARCHIVE_POSTFIX + compress_suffix, "trajectory": TRAJ_POSTFIX}
    ...
    if log_par.should_render(name):
        for i, traj_id in enumerate(sil_trajectories):
            if traj_id in expl.archive.cell_trajectory_manager.cell_trajectories:
                trajectory = expl.archive.cell_trajectory_manager.get_trajectory(traj_id, -1, expl.archive.cell_id_to_key_dict)
                cells = dict()
                cells_count = dict()
...
class CheckpointTracker:
    ...
    def calc_write_checkpoint(self):
        if self.log_par.checkpoint_compute is not None:
            passed_compute_thresh = (
                self.old_compute // self.log_par.checkpoint_compute
        ...
        if self.log_par.checkpoint_it is not None:
            passed_it_thresh = self.old_it // self.log_par.checkpoint_it != self.expl.cycles // self.log_par.checkpoint_it
        else:
        ...
        if self.log_par.checkpoint_time is not None:
            passed_time_thresh = (
                self.old_time_passed // self.log_par.checkpoint_time
                != self.current_time_passed // self.log_par.checkpoint_time
            )
        else:
            passed_time_thresh = False

        return first_it or passed_compute_thresh or passed_it_thresh or last_it or self.will_log_warmup or passed_time_thresh
    ...
    while checkpoint_tracker.should_continue():
        ...
        if hvd.rank() == 0 and not disable_logging:
            ...
            new_trajectories = sorted(
                traj_manager.new_trajectories, key=lambda t: traj_manager.cell_trajectories[t].frame_finished
            )
