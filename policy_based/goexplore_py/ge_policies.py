...
class GRUPolicyGoalConSimpleFlexEnt(object):
    def __init__(self, sess, ob_space, ac_space, nenv, nsteps, memsize=800, test_mode=False, reuse=False, goal_space=None):
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            h = tf.nn.relu(po.conv(tf.cast(nn_input, tf.float32) / 255.0, "c1", noutchannels=64, filtsize=8, stride=4))
            ...
            h4 = tf.contrib.layers.layer_norm(
                po.fc(h3, "fc1", nout=memsize), center=False, scale=False, activation_fn=tf.nn.relu
            )
        ...
        def step(local_ob, local_goal, local_state, local_mask, local_increase_ent):
            return sess.run(
                [a0, vf, snew, neglogp0],
                {nn_input: local_ob, states: local_state, mask: local_mask, entropy: local_increase_ent, goal: local_goal},
            )

        def step_fake_action(local_ob, local_goal, local_state, local_mask, local_increase_ent, local_fake_action):
            return sess.run(
                [a0, vf, snew, neglogp0, neg_log_fake_a],
                {
                    nn_input: local_ob,
                    states: local_state,
                    mask: local_mask,
