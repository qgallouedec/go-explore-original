...
def get_game(
    ...
    if game_lowered == "montezuma" or game_lowered == "montezumarevenge":
        ...
        grid_resolution = (
            ...
            GridDimension("objects", 1),
            GridDimension("room", 1),
            GridDimension("x", x_res),
            GridDimension("y", y_res),
        )
        ...
        game_class.TARGET_SHAPE = target_shape
        game_class.MAX_PIX_VALUE = max_pix_value
        game_args = dict(cell_representation=cell_representation)
        grid_resolution = (
            GridDimension("level", 1),
            GridDimension("objects", 1),
            GridDimension("room", 1),
        ...
        game_args = dict(env_name=game.split("_")[1], interval_size=interval_size, seed_low=seed_low, seed_high=seed_high)
        grid_resolution = (
            GridDimension("level", 1),
            GridDimension("objects", 1),
            GridDimension("room", 1),
...
def hrv_and_tf_init(nb_cpu, nb_envs, seed_offset):
    ...
    config = tf.ConfigProto(
        allow_soft_placement=True, intra_op_parallelism_threads=nb_cpu, inter_op_parallelism_threads=nb_cpu
    )
    ...
    for archive_name in archive_names.split(":"):
        if archive_name.lower() == "domainknowledge":
            domain_knowledge_archive = archives.DomainKnowledgeArchive(
                optimize_score, selector, cell_trajectory_manager, grid_resolution, max_failed, reset_on_update
            )
...
def get_goal_rep(
    ...
    if goal_representation_name == "raw":
        goal_representation = goal_rep.ScaledGoalRep(rep_type, rel_final_goal, rel_sub_goal, cell_representation.array_length)
        policy_name = "gru_simple_goal"
    elif goal_representation_name == "scaled_1_1_1":
        goal_representation = goal_rep.ScaledGoalRep(
            rep_type, rel_final_goal, rel_sub_goal, cell_representation.array_length, norm_const=[1, 1, 1, 20, 10]
        )
        policy_name = "gru_simple_goal"
    elif goal_representation_name == "scaled":
        goal_representation = goal_rep.ScaledGoalRep(
            rep_type, rel_final_goal, rel_sub_goal, cell_representation.array_length, norm_const=[3, 3, 24, 20, 10]
        )
        policy_name = "gru_simple_goal"
        ...
        goal_representation = goal_rep.OneHotGoalRep(
            rep_type, rel_final_goal, rel_sub_goal, cell_representation.get_max_values()
        )
        policy_name = "gru_simple_goal"
...
def setup(
    ...
    # Get game
    game_name, game_class, game_args, grid_resolution = get_game(
        game=game,
    ...
    if selector_name == "weighted":
        selector = randselectors.WeightedSelector(
            selector_weights_list, selector_special_attribute_list, base_weight, weight_based_skew
        )
    ...
    cell_trajectory_manager = CellTrajectoryManager(
        sil, cell_representation, temp_dir=temp_dir, low_prob_traj_tresh=low_prob_traj_tresh
    )
    ...
    if trajectory_tracker_name == "dummy":
        ...
        trajectory_tracker = trajectory_trackers.DelayedSoftTrajectoryTracker(cell_reached, soft_traj_track_window_size, delay)
    ...
    goal_representation, policy_name = get_goal_rep(
    ...
    runner = runner_class(
        env=env, model=model, nsteps=num_steps, gamma=gamma, lam=lam, norm_adv=norm_adv, subtract_rew_avg=subtract_rew_avg
    )

    logger.info("Creating gatherer")
    trajectory_gatherer = trajectory_gatherers.StochasticGatherer(
        env=env,
...
def safe_set_argument(args, attr, value):
    ...
    if not hasattr(args, attr):
        ...
        raise RuntimeError("Argument " + value + " has already been provided as an option, and should not be set here.")
...
def parse_arguments():
    ...
    # General arguments
    parser.add_argument("--seed", type=int, default=DefaultArg(0), help="The random seed.")
    parser.add_argument(
        "--profile",
        dest="profile",
        action="store_true",
        default=DefaultArg(False),
    ...
    parser.add_argument(
        ...
        help="Shortcut for passing --resize_x (0), --resize_y (1) and --max_pix_value (2) all at the " "same time: 0x1x2",
    )

    # I/O Arguments
    parser.add_argument(
        "--base_path", "-p", type=str, default=DefaultArg("./results/"), help="Folder in which to store results"
    )
    parser.add_argument("--load_path", type=str, default=DefaultArg(""), help="Folder from which to load a pre-trained model.")
    parser.add_argument(
        "--fail_on_duplicate",
        default=DefaultArg(False),
    ...
    parser.add_argument(
        "--end_on_death", dest="end_on_death", default=DefaultArg(False), action="store_true", help="End episode on death."
    )
    parser.add_argument(
        "--game", "-g", type=str, default=DefaultArg("montezuma"), help="Determines the game to which apply goexplore."
    )
    parser.add_argument("--interval_size", type=float, default=DefaultArg(0.1), help="The interval size for robotics envs.")
    parser.add_argument(
        "--sticky_actions",
        default=DefaultArg(False),
        dest="sticky",
    ...
    parser.add_argument(
        "--noops", default=DefaultArg(False), dest="noops", action="store_true", help="Whether to run with noops actions."
    )

    # Stopping criteria arguments
    parser.add_argument("--max_game_steps", type=int, default=DefaultArg(None), help="Maximum number of GAME frames.")
    parser.add_argument(
        "--max_compute_steps", "--mcs", type=int, default=DefaultArg(None), help="Maximum number of COMPUTE frames."
    )
    parser.add_argument("--max_iterations", type=int, default=DefaultArg(None), help="Maximum number of iterations.")
    parser.add_argument(
        "--max_hours", "--mh", type=float, default=DefaultArg(None), help="Maximum number of hours to run this for."
    )
    parser.add_argument("--max_cells", type=int, default=DefaultArg(None), help="The maximum number of cells before stopping.")
    parser.add_argument(
        "--max_score",
        type=float,
        default=DefaultArg(None),
    ...
    parser.add_argument(
        "--checkpoint_compute", type=int, default=DefaultArg(None), help="Save a checkpoint every this many COMPUTE frames."
    )
    parser.add_argument(
        "--checkpoint_it", type=int, default=DefaultArg(None), help="Save a checkpoint every this many iterations."
    )
    parser.add_argument(
        "--checkpoint_time", type=int, default=DefaultArg(None), help="Save a checkpoint every this many seconds."
    )
    parser.add_argument(
        "--clear_checkpoints",
    ...
    parser.add_argument(
        "--pictures", dest="save_pictures", type=str, default=DefaultArg(""), help="Save pictures of the specified checkpoints"
    )
    parser.add_argument(
        "--clear_pictures",
        dest="clear_pictures",
    ...
    parser.add_argument(
        ...
        dest="archive_names",
        type=str,
        default=DefaultArg("DomainKnowledge"),
        help="What archives to use. " "The first archive in this colon-separated list will be the active archive.",
    )
    parser.add_argument(
        "--no_optimize_score",
        dest="optimize_score",
    ...
    parser.add_argument(
        ...
        dest="frame_resize",
        type=str,
        default=DefaultArg("RectColorFrame"),
        help="How to resize the frame, and whether the frame will be color or grey scale.",
    )
    parser.add_argument(
        "--frame_history", dest="frame_history", type=int, default=DefaultArg(4), help="How many old frames to show."
    )
    ...
    parser.add_argument(
        "--trajectory_tracker",
        dest="trajectory_tracker_name",
        type=str,
        default=DefaultArg("dummy"),
    ...
    parser.add_argument(
        ...
        dest="selector_name",
        type=str,
        default=DefaultArg("random"),
        help="How to select cells to return to. " "Options are: random, mr_curriculum, iterative, and weighted.",
    )
    parser.add_argument(
        "--selector_weights",
        dest="selector_weights_str",
        type=str,
        default=DefaultArg(""),
    ...
    parser.add_argument(
        "--max_exploration_steps",
        dest="max_exploration_steps",
        type=int,
        default=DefaultArg(100),
    ...
    # PPO arguments
    parser.add_argument(
        "--nb_envs", type=int, default=DefaultArg(16), help="The number of environments to run in parallel with Horovod."
    )
    parser.add_argument(
        "--clip_rewards",
        default=DefaultArg(False),
    ...
    parser.add_argument("--gamma", type=float, default=DefaultArg(0.999), help="The discount factor.")
    parser.add_argument(
        "--freeze_network",
        default=DefaultArg(False),
        dest="freeze_network",
    ...
    parser.add_argument(
        "--expl_inc_ent_thresh",
        dest="expl_inc_ent_thresh",
        type=int,
        default=DefaultArg(-1),
        help="The number of steps of exploration towards a particular goal after which entropy will " "be increased.",
    )
    parser.add_argument(
        "--ret_inc_ent_fac",
        dest="ret_inc_ent_fac",
        type=float,
        default=DefaultArg(1),
    ...
    parser.add_argument(
        ...
        dest="expl_ent_reset",
        type=str,
        default=DefaultArg("on_new_cell"),
        help="When should we reset the entropy while exploring.",
    )
    parser.add_argument("--sil", dest="sil", type=str, default=DefaultArg("none"), help="Turn self-imitation learning on.")
    parser.add_argument(
        "--sil_coef",
        dest="sil_coef",
        type=float,
        default=DefaultArg(0.1),
    ...
    parser.add_argument(
        "--random_exp_prob",
        dest="random_exp_prob",
        type=float,
        default=DefaultArg(0),
        help="The probability that we will be taking random actions," " rather than sampling from the policy.",
    )
    parser.add_argument(
        "--mean_repeat",
        dest="mean_repeat",
        type=int,
        default=DefaultArg(20),
        help="The expected number of times that an action will be repeated by the random explorer.",
    )
    parser.add_argument(
        "--checkpoint_first_iteration",
        dest="checkpoint_first_iteration",
        type=int,
        default=DefaultArg(1),
    ...
    parser.add_argument(
        "--checkpoint_last_iteration",
        dest="checkpoint_last_iteration",
        type=int,
        default=DefaultArg(1),
    ...
    parser.add_argument(
        "--learning_rate", dest="learning_rate", type=float, default=DefaultArg(1e-4), help="Sets the learning rate"
    )
    parser.add_argument(
        "--cell_representation",
        dest="cell_representation_name",
        type=str,
        default=DefaultArg("level_room_keys_x_y"),
        help="The cell representation to use.",
    )
    parser.add_argument(
        "--game_reward_factor",
        dest="game_reward_factor",
        type=float,
    ...
    parser.add_argument(
        "--goal_reward_factor",
        dest="goal_reward_factor",
        type=float,
        default=DefaultArg(1.0),
        help="The factor by which to multiply the goal reward",
    )
    parser.add_argument(
        "--clip_game_reward",
        dest="clip_game_reward",
        type=int,
        default=DefaultArg(1),
    ...
    parser.add_argument(
        "--cell_reached",
        dest="cell_reached_name",
        type=str,
        default=DefaultArg("equal"),
        help="How to determine whether a cell is considered reached when following a trajectory.",
    )
    parser.add_argument(
        "--soft_traj_win_size",
        dest="soft_traj_track_window_size",
        type=int,
        default=DefaultArg(10),
        help="The size of the soft-trajectory window",
    )
    parser.add_argument(
        "--load_archive", dest="archive_to_load", type=str, default=DefaultArg(None), help="The archive to load"
    )
    parser.add_argument(
        "--expl_state", dest="expl_state", type=str, default=DefaultArg(None), help="The go-explore state to load"
    )
    parser.add_argument(
        "--warm_up_cycles",
        dest="warm_up_cycles",
    ...
    parser.add_argument(
        "--nb_of_epochs", dest="nb_of_epochs", type=int, default=DefaultArg(4), help="The number of PPO epochs"
    )
    parser.add_argument(
        "--legacy_entropy",
        dest="legacy_entropy",
    ...
    parser.add_argument(
        "--screenshot_merge", dest="screenshot_merge", type=str, default=DefaultArg("mpi"), help="How to merge screenshots."
    )
    parser.add_argument(
        "--final_goal_reward",
        dest="final_goal_reward",
        type=float,
        default=DefaultArg(1.0),
    ...
    parser.add_argument(
        "--low_prob_traj_tresh",
        dest="low_prob_traj_tresh",
        type=float,
        default=DefaultArg(0.00001),
        help="The probability below which trajectories will be written to disk.",
    )
    parser.add_argument(
        "--reset_on_update",
        dest="reset_on_update",
        type=int,
        default=DefaultArg(0),
    ...
    safe_set_argument(args, "lam", DefaultArg(0.95))
