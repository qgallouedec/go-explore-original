...
class Explore:
    def __init__(self, explorer_policy, cell_selector, env, grid_info: tuple, pool_class, args, important_attrs):
        global POOL, ENV
        self.args = args
    ...
    def maybe_split_dynamic_state(self):
        if (
            self.frames_compute - self.last_recompute_dynamic_state > self.args.recompute_dynamic_state_every
    ...
    def process_cell(self, info):
        ...
        return TimedPickle(
            (cell_key, end_trajectory, self.frames_true, self.frames_compute, known_room_data), "ret", enabled=info.enabled
        )
    ...
    def sample_only_cycle(self):
        ...
        for i, cell_key in enumerate(cell_keys):
            ...
            chosen_cells.append(
                TimedPickle(
                    (cell_key, cell_copy, seed, len(ENV.rooms), self.env_info[0].TARGET_SHAPE, self.env_info[0].MAX_PIX_VALUE),
                    "args",
        ...
        for ((cell_key, cell_copy, seed, _, _, _), (_, end_trajectory, ft, fc, known_rooms)) in zip(
            chosen_cells, trajectories
        ):
        ...
        for ((cell_key, cell_copy, seed, _, _, _), (_, end_trajectory, ft, fc, known_rooms)) in zip(
            ...
            for i, elem in enumerate(end_trajectory):
                if "fetch" in self.args.game:
                    self.gripped_info_count[elem.to.exact.gripped_info] = (
                        self.gripped_info_count.get(elem.to.exact.gripped_info, 0) + 1
                    )
                ...
                if (elem.to.restore is not None or potential_cell_key == DONE) and self.should_accept_cell(
    ...
    def should_accept_cell(self, potential_cell, cur_score, full_traj_len):
        ...
        if self.args.optimize_score:
            return cur_score > potential_cell.score or (
                full_traj_len < potential_cell.trajectory_len and cur_score == potential_cell.score
            )
    ...
    def save_checkpoint(self, suffix=""):
        ...
        if self.args.save_pictures or self.args.save_item_pictures or self.args.save_prob_pictures:
            ...
            if not self.args.use_real_pos:
                object_combinations = sorted(set(e.real_cell.score for e in self.grid.values() if e.real_cell is not None))
                for obj in object_combinations:
                    grid_at_obj = [
                        e.real_cell for e in self.grid.values() if e.real_cell is not None and e.real_cell.score == obj
                    ]
                    get_env().render_with_known(
                        grid_at_obj,
                        self.args.resolution,
            ...
            if (self.args.use_real_pos and self.args.save_pictures) or self.args.save_prob_pictures:
                ...
                total = np.sum([self.selector.get_weight(x, self.grid[x], possible_scores, self.grid) for x in self.grid])
                get_env().render_with_known(
                    list(self.grid.keys()),
                    self.args.resolution,
                    show=False,
                    ...
                    combine_val=lambda x, y: x + y,
                    get_val=lambda x: self.selector.get_weight(x, self.grid[x], possible_scores, self.grid) / total,
                )
