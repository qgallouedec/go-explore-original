...
def _run(base_path, args):
    ...
    if args.game == "montezuma":
        ...
        game_args = dict(
            score_objects=args.use_objects,
            x_repeat=args.x_repeat,
        ...
        grid_resolution = (
            ...
            GridDimension("score", 1),
            GridDimension("room", 1),
            GridDimension("x", args.resolution),
            GridDimension("y", args.resolution),
        )
        ...
        game_args = dict(
            env_name=args.game.split("_")[1], interval_size=args.interval_size, seed_low=args.seed, seed_high=args.seed
        )
        grid_resolution = (
            GridDimension("level", 1),
            GridDimension("score", 1),
        ...
        if args.target_location == "0010" or (args.target_location is None and not args.target_single_shelf):  # Lower door
            ...
            door1_dists = FetchConditionalObject("0010", door1_dists_use, door1_dists_ignore)
        else:
        ...
        if args.target_location == "0001" or (args.target_location is None and not args.target_single_shelf):  # Lower door
            ...
            door_dists = FetchConditionalObject("0001", door_dists_use, door_dists_ignore)
        else:
        ...
        grid_resolution = (
            door_dists,
            door1_dists,
            GridDimension("gripped_info", 1),
            GridDimension("gripped_pos", 1000, 500),
            target_grid,
            GridDimension("gripper_pos", args.gripper_pos_resolution),
        )
        if args.fetch_single_cell:
            grid_resolution = (
                SingleCell("door_dists", (0,)),
                SingleCell("door1_dists", (0,)),
                SingleCell("gripped_info", None),
                SingleCell("gripped_pos", None),
                SingleCell("object_pos", None),
                SingleCell("gripper_pos", None),
            )
    ...
    if args.seed_path is not None:
        ...
        (
            expl.experience_prev_ids,
            expl.experience_actions,
            expl.experience_rewards,
            expl.experience_cells,
            expl.experience_scores,
            expl.experience_lens,
        ) = exp_data
        expl.dynamic_state_split_rules = [e[0] for e in expl.experience_cells if isinstance(e, tuple)][-1]
    ...
    with tqdm(desc="Time (seconds)", smoothing=0, total=MAX_TIME) as t_time, tqdm(
        desc="Iterations", total=MAX_ITERATIONS
    ) as t_iter, tqdm(desc="Compute steps", total=MAX_FRAMES_COMPUTE) as t_compute, tqdm(
        desc="Game step", total=MAX_FRAMES
    ) as t, tqdm(
        desc="Max score", total=MAX_SCORE
    ) as t_score, tqdm(
        desc="Done score", total=MAX_SCORE
    ) as t_done_score, tqdm(
        desc="Cells", total=MAX_CELLS
    ) as t_cells:
        ...
        while should_continue():
            ...
            if (
                (not seen_level_1 and expl.seen_level_1)
                or old == 0  # We have solved level 1
                or old // THRESH_TRUE != expl.frames_true // THRESH_TRUE  # It is the first iteration
                or old_compute // THRESH_COMPUTE  # We just passed the THRESH_TRUE threshold
                != expl.frames_compute // THRESH_COMPUTE
                or not should_continue()  # We just passed the THRESH_COMPUTE threshold
...
def display_top(snapshot, key_type="traceback", limit=20):
    ...
    for index, stat in reversed(list(enumerate(top_stats[:limit], 1))):
        ...
        tqdm.write("#%s: %s:%s: %.1f MB" % (index, filename, frame.lineno, stat.size / (1024 * 1024)))
...
def run(base_path, args):
    ...
    shutil.copytree(
        cur_dir, code_path, ignore=shutil.ignore_patterns("*.png", "*.stl", "*.JPG", "__pycache__", "LICENSE*", "README*")
    )
...
if __name__ == "__main__":
    ...
    boolarg(
        "--optimize_score",
        default=True,
        help='Optimize for score (only speed). Will use fewer "game frames" and come up with faster trajectories with lower scores. If not combined with --remember_rooms and --objects_from_ram is not enabled, things should run much slower.',
    )
    add_argument(
        "--prob_override",
        type=float,
    ...
    boolarg(
        "--save_cells",
        default=False,
        help="Save exact cells produced by Go-Explore instead of just hints as to whether they are done or not.",
    )
    ...
    add_argument(
        "--max_score", type=float, default=None, help="Stop when this score (or more) has been reached in the archive."
    )

    current_group = parser.add_argument_group("General Selection Probability")
    add_argument(
        "--seen_weight", "--sw", type=float, default=0.0, help='The weight of the "seen" attribute in cell selection.'
    )
    add_argument("--seen_power", "--sp", type=float, default=0.5, help='The power of the "seen" attribute in cell selection.')
    add_argument(
        "--chosen_weight", "--cw", type=float, default=0.0, help='The weight of the "chosen" attribute in cell selection.'
    )
    add_argument(
        "--chosen_power", "--cp", type=float, default=0.5, help='The power of the "chosen" attribute in cell selection.'
    )
    add_argument(
    ...
    add_argument(
        "--action_weight", "--aw", type=float, default=0.0, help='The weight of the "action" attribute in cell selection.'
    )
    add_argument(
        "--action_power", "--ap", type=float, default=0.5, help='The power of the "action" attribute in cell selection.'
    )
    ...
    add_argument(
        "--low_score_weight", type=float, default=0.0, help="Weight of not having a neighbor with a lower score/object number."
    )
    add_argument(
        "--high_score_weight",
        type=float,
    ...
    boolarg(
        ...
        neg="--use_scores",
        default=True,
        help="Use objects in the cell description. Otherwise scores will be used.",
    )
    add_argument(
        "--x_repeat",
        type=int,
    ...
    boolarg(
        "--objects_from_pixels", neg="--objects_from_ram", default=True, help="Get the objects from pixels instead of RAM."
    )
    ...
    add_argument(
        "--pitfall_treasure_type",
        type=str,
        default="none",
        help="How to include treasures in the cell description of Pitfall: none (don't include treasures), count (include treasure count), score (include sum of positive rewards) or location (include the specific location the treasures were found).",
    )

    current_group = parser.add_argument_group("Atari No Domain Knowledge")
    boolarg(
        "--use_real_pos",
        neg=["--state_is_pixels", "--pix"],
        default=True,
        help="If this is on, the state will be resized pixels, not human prior.",
    )
    add_argument(
        "--resize_x", "--rx", type=int, default=11, help="What to resize the pixels to in the x direction for use as a state."
    )
    add_argument(
        "--resize_y", "--ry", type=int, default=8, help="What to resize the pixels to in the y direction for use as a state."
    )
    add_argument(
        "--max_pix_value",
        "--mpv",
    ...
    boolarg(
        "--dynamic_state",
        help="Dynamic downscaling of states. Ignores --resize_x, --resize_y, --max_pix_value and --resize_shape.",
    )
    ...
    add_argument(
        ...
        type=int,
        default=100_000,
        help="Number of steps before recomputing the dynamic state representation (ignored if negative).",
    )
    add_argument(
    ...
    add_argument(
        ...
        type=float,
        default=0.03,
        help="The factor by which we try to split frames when recomputing the representation. 1 -> each frame is its own cell. 0 -> all frames are in the same cell.",
    )
    add_argument(
        "--split_iterations",
        type=int,
        default=100,
        help="The number of iterations when recomputing the representation. A higher number means a more accurate (but less stochastic) results, and a lower number means a more stochastic and less accurate result. Note that stochasticity can be a good thing here as it makes it harder to get stuck.",
    )
    add_argument(
        "--max_recent_frames",
        type=int,
        default=5_000,
        help="The number of recent frames to use in recomputing the representation. A higher number means slower recomputation but more accuracy, a lower number is faster and more stochastic.",
    )
    add_argument(
        "--recent_frame_add_prob",
        type=float,
        default=0.1,
        help="The probability for a frame to be added to the list of recent frames.",
    )
    ...
    add_argument(
        ...
        type=int,
        default=20,
        help="The number of substeps in mujoco between each action (each substep takes 0.002 seconds).",
    )
    ...
    add_argument(
        ...
        type=int,
        default=0,
        help="The minimum grip score (inclusive) for a fetch grip to be included in the archive.\n0: at least 1 finger touching, 1: 2 fingers touching, 3: 2 fingers touching AND not touching the table (gripping and lifting).",
    )
    add_argument(
        "--max_grip_score",
        type=int,
        default=3,
        help="The maximum grip score (inclusive). All grips with higher scores will be given this score instead.",
    )
    add_argument(
        "--minmax_grip_score",
        type=str,
        default=None,
        help="Shortcut to set both the min and max grip score. The first digit is the min and second is the max.",
    )
    ...
    add_argument(
        ...
        type=float,
        default=None,
        help="The total timestep length (if included, timestep is ignored from the command line and instead set to total_timestep / nsubsteps). A reasonable value is 0.08",
    )
    ...
    boolarg(
        "--fetch_uniform",
        help="Select uniformly for fetch. Shurtcut for --door_weight=0 --grip_weight=0 --low_level_weight=1.",
    )
    boolarg(
        "--conflate_objects",
        help="Conflate objects when getting their positions. With this, there is no difference between object 1 being in shelf 0001 and object 2 being in shelf 0001.",
    )
    ...
    boolarg(
        "--combine_table_shelf_box",
        help="Combine the table and shelf box for determining death by being outside the table-shelf box.",
    )
