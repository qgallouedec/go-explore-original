        ...
        if cmd == "step":
            ...
            for step in range(count):
                ...
                if "episode" in info:
                    epinfo = info["episode"]
                    print(
                        "simulator thread %d completed demo run with total return %d obtained in %d steps"
                        % (nr, epinfo["r"], epinfo["l"])
                    )
                obs.append(ob)
                rews.append(reward)
                dones.append(done)
...
class ClonedVecEnv(object):
    def __init__(self, env_fns, mode="best"):
        ...
        self.ps = [
            Process(target=worker2, args=(nr, work_remote, CloudpickleWrapper(env_fn), mode))
